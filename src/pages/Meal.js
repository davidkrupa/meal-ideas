...
const Meal = () => {
  ...
  const ingredientsKeys = Object.keys(details).filter(
    (key) =>
  ...
  const sentences = details.strInstructions
    ?.split(".")
  ...
  if (sentences) {
    for (let i = 0; i < sentences.length; i++) {
      if (i === 0) {
        ...
        paragraphs[paragraphs.length - 1].length + sentences[i].length <
        paragraphLength
      ) {
        paragraphs[paragraphs.length - 1] =
          paragraphs[paragraphs.length - 1] + sentences[i];
      } else {
    ...
    <Stack alignItems="center" mt="50px" spacing={7}>
      ...
      >
        ...
        <Stack spacing={3}>
          ...
          <Stack pl="15px">{ingredients}</Stack>
        </Stack>
      </Stack>
